...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'interact'` in [`GameService.execute`](../src/web/game.ts) and `GameService.interact` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'investigate'` in [`GameService.execute`](../src/web/game.ts) and `GameService.investigate` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'move'` in [`GameService.execute`](../src/web/game.ts) and `GameService.move` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'ride'` in [`GameService.execute`](../src/web/game.ts) and `GameService.ride` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'fold'` in [`GameService.execute`](../src/web/game.ts) and `GameService.fold` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'passage'` in [`GameService.execute`](../src/web/game.ts) and `GameService.passage` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'oath'` in [`GameService.execute`](../src/web/game.ts) and `GameService.oath` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'cultivate'` in [`GameService.execute`](../src/web/game.ts) and `GameService.runSeclusion` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'seclude'` in [`GameService.execute`](../src/web/game.ts) and `GameService.runSeclusion` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'breakthrough'` in [`GameService.execute`](../src/web/game.ts) and `GameService.strikeBarrier` · the deterministic parser reaches it · spends in-world time.

### `train_technique`

practise a specific art the cultivator already knows. "target" names it.

Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'train_technique'` in [`GameService.execute`](../src/web/game.ts) and `GameService.train` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'refine'` in [`GameService.execute`](../src/web/game.ts) and `GameService.refine` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'gather'` in [`GameService.execute`](../src/web/game.ts) and `GameService.gather` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'hunt'` in [`GameService.execute`](../src/web/game.ts) and `GameService.hunt` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'eat'` in [`GameService.execute`](../src/web/game.ts) and `GameService.eat` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'treat'` in [`GameService.execute`](../src/web/game.ts) and `GameService.treat` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'buy'` in [`GameService.execute`](../src/web/game.ts) and `GameService.buy` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'inventory'` in [`GameService.execute`](../src/web/game.ts) and `GameService.inventory` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'consume_pill'` in [`GameService.execute`](../src/web/game.ts) and `GameService.consumePill` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'list_techniques'` in [`GameService.execute`](../src/web/game.ts) and `GameService.listTechniques` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'learn_technique'` in [`GameService.execute`](../src/web/game.ts) and `GameService.learnTechnique` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'acquisition'` in [`GameService.execute`](../src/web/game.ts) and `GameService.guidance` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'ceiling'` in [`GameService.execute`](../src/web/game.ts) and `GameService.ceiling` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'teacher'` in [`GameService.execute`](../src/web/game.ts) and `GameService.teacher` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'destinations'` in [`GameService.execute`](../src/web/game.ts) and `GameService.destinations` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'roads'` in [`GameService.execute`](../src/web/game.ts) and `GameService.loadWorld` · the deterministic parser reaches it · passes no time.

### `wait`

let a day go by doing nothing in particular.

Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'wait'` in [`GameService.execute`](../src/web/game.ts) and `GameService.shortSkip` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'work'` in [`GameService.execute`](../src/web/game.ts) and `GameService.work` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'market'` in [`GameService.execute`](../src/web/game.ts) and `GameService.market` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'sect'` in [`GameService.execute`](../src/web/game.ts) and `GameService.sect` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'site'` in [`GameService.execute`](../src/web/game.ts) and `GameService.site` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'legacy'` in [`GameService.execute`](../src/web/game.ts) and `GameService.legacyAct` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'petition'` in [`GameService.execute`](../src/web/game.ts) and `GameService.petition` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'posture'` in [`GameService.execute`](../src/web/game.ts) and `GameService.posture` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'seal'` in [`GameService.execute`](../src/web/game.ts) and `GameService.seal` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'offer'` in [`GameService.execute`](../src/web/game.ts) and `GameService.offer` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'descend'` in [`GameService.execute`](../src/web/game.ts) and `GameService.descend` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves at `case 'look'` in [`GameService.execute`](../src/web/game.ts) · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'status'` in [`GameService.execute`](../src/web/game.ts) and `GameService.freeAction` · the deterministic parser reaches it · passes no time.

### `assess`

what would happen if they tried something: the odds, not the attempt. "target" names the place or the opponent.

Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'assess'` in [`GameService.execute`](../src/web/game.ts) and `GameService.assess` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'recall'` in [`GameService.execute`](../src/web/game.ts) and `GameService.recall` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'recognise'` in [`GameService.execute`](../src/web/game.ts) and `GameService.recognise` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'news'` in [`GameService.execute`](../src/web/game.ts) and `GameService.news` · the deterministic parser reaches it · passes no time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'request'` in [`GameService.execute`](../src/web/game.ts) and `GameService.request` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'propose'` in [`GameService.execute`](../src/web/game.ts) and `GameService.proposeAMatch` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'decline'` in [`GameService.execute`](../src/web/game.ts) and `GameService.declineAMatch` · the deterministic parser reaches it.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'child'` in [`GameService.execute`](../src/web/game.ts) and `GameService.haveAChild` · the deterministic parser reaches it · spends in-world time.
...
Declared in [`ACTION_NAMES`](../src/web/actions.ts) · resolves through `case 'unclear'` in [`GameService.execute`](../src/web/game.ts) and `GameService.guidance` · what the deterministic parser answers when nothing else matched · passes no time.
