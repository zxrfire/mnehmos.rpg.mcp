...
export const RECOVERED_RECIPE_IDS: ReadonlySet<string> = new Set([
    ...
    // Recovered in the ordinary way and then not secret at all. This formula
    // circulates freely among the houses that hold a copy, because a method
    // whose first ingredient stopped growing is not worth guarding - which is
    // what makes the loss legible rather than mysterious, and is the whole
    // difference between a lost medicine and a lost art.
...
const RECIPE_DATA: readonly Recipe[] = [
    ...
    },
    {
        ...
        // against `RecipeSchema`, all three ingredients resolve to real herbs,
        ...
        // holds. A player can read it, cost it, and go shopping.
        //
        // They will not find the first ingredient, because it is in
        ...
        // set membership in `herbs.ts`, no rule in this file, and no branch in
        // the refiner. The knowledge survived; the input did not.
        //
        // THE CHAOS SEED CAME OFF, deliberately. It used to head this list, and
        // it is the wrong kind of scarce: four are known and three are
        // accounted for, so it is rare in the way a famous object is rare.
        // Of the flower, zero are known. A formula with two impossible
        // ingredients says less than one with a single impossible ingredient,
        // because the second one blurs which absence is doing the work.
