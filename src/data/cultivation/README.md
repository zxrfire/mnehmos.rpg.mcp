...
| `places-that-teach-a-dao.ts` | Named ground where a road besides your own can be walked, and who is standing on the door | Nothing here teaches `element` - the one road a root supplies unaided - and no house holds the only door to any domain. Three access kinds, three different sentences about why you cannot get in: standing, geography, and nobody has dug it out yet. `fromOrdinal` is a floor and never a ceiling, because a principle is either legible to you or it is not |
...
| `what-each-house-makes-and-what-crosses-the-water.ts` | Who makes what, the named sea lanes, what is on the hulls, who carries it, and the free port's terms | Artisans are a FACT about factions and never a production system: `artisansOf` falls back to the province's own `exports`, so every house has makers without a table of thirty-four rows. Every cargo row resolves to a maker or to ground - a good with no origin is a second economy beside the market |
