...
export const DEATHS_AVAILABLE = {
    ...
    whyNobodyHasDoneIt:
        'It has never been assembled, and the reason is not scruple. The conversation cannot be had without being reported, an apex that hears the first half does not wait for the second, and the Earth Vein Tower in particular holds an instrument that makes concealment structurally difficult - so the plot must be built against the very object it is being built to take. What protects the top of the world is not that it cannot be killed. It is that killing it requires several parties to trust each other completely, once, with no way to verify, against somebody who is very hard to lie to.',
...
export const WHY_NOBODY_MOVES = {
    ...
    andItHasHappenedOnce:
        'Deeproot Court answered the Earth Vein Tower for nine hundred years and does not any more. The Survey reposted it - which is a thing you can do to a posting and to nothing else in the world - and most of the Wardens declined the reposting and walked, and the Myriad Course Hall offered them a schedule and they took it. No fighting, no announcement, no betrayal anybody could name. What went with them was not a grant book: it was the roll, the founding posting order, and a claim on the strongest sealed thing anybody has established the existence of, which the Myriad Course Hall acquired by taking in some disaffected appointees and never having to say a word. It is the largest thing that has happened at the top of the world in nine centuries and there is no battle in it anywhere.',
    andThePavilionIsTheReasonNEITHEROfThemMoves:
        'The Azure Cloud Pavilion is the shallowest house on the board rather than the emptiest one - one body at the last realm, the next name three full stages below it, and a single four-person court against the others\' courts and client rings - and either of the other two can take it in a day - the Survey ninety-nine times in a hundred, the Myriad Course Hall a hundred. Neither will, and the reason is not mercy. Whoever does it walks out of that mountain having spent a war on a house at forty-one holding the most martial object in the world, and then has to stand in front of the third apex the same season: measured, the Survey survives that meeting twice in a hundred and the Myriad Course Hall not once. Taking the weakest house in the region is the single most reliable way to stop existing that either of them has available.',
    ...
    andAPersonIsNotWorthNothingAfterAll:
        ...
        'The catalog used to say a person was worth almost nothing at this altitude and an object was worth the region, which was tidy and is half wrong. Give the Earth Vein Tower a second cultivator at forty-four and its best move moves from nothing to three in a thousand - almost nothing, exactly as claimed. Give the LONG CUT one and it takes the Pavilion sixty-five times in a hundred and the Survey thirty-five. Same gift, same rung, two houses, and the difference is not close.',
    andWhatDecidesItIsHowManyTheyAlreadyHave:
        'What separates them is what the gift lands on. The Earth Vein Tower is one seated person and an instrument, and a second body standing next to somebody who has not left a room in four hundred years does not change what the Survey is; it mobilises eight and the eighth is not the problem. The Myriad Course Hall mobilises fifteen, and a fifteenth thing at the last realm arrives on top of a position that was already winning its wars and losing only the evening afterwards - which is precisely the margin a second body closes. So an object is worth more than a person to everybody, and how much more depends entirely on how many people you had already. That is not the neat claim. It is the one that is true, and it explains something the neat one never could: why the two of them watch each other\'s grant books more closely than each other\'s vaults, which this file has always said and could not previously account for.',
...
export const THE_REVOLT = {
    ...
    andTheOneThatWouldNotWork:
        'The Myriad Course Hall is the exception and it is instructive. Twelve of its own turning on it takes the head none times in a hundred, against nine taking the Earth Vein Tower\'s ninety-five, and the twelve are the larger number. What decides it is the Ninth Nail: forty-five in the hands of the person being revolted against, against a Datum Lamp at forty-three in the other case. The most dangerous thing about a head is not their rung and never was - it is what they are holding, and a house whose people might one day come up the stairs is a house that should think carefully about what it lets its head keep.',
...
export const THE_COURTS_BELOW = {
    ...
    andTheyAllClaimItAtOnce:
        'Which is the problem, because there is more than one of them. The Earth Vein Tower alone has three, each administering its own arterial, each with the same argument and none with a better one. So the succession is not a vacuum and it is not a war of conquest either - it is three institutions issuing letters, honouring each other\'s grants selectively, and waiting to see which of them the client sects keep paying. The first court that another court\'s clients start paying has won, and it will have happened without anybody drawing anything.',
    whatSettlesIt:
...
export const OPENLY_OR_IN_SECRET = {
    theAllianceIsVisible:
        'An alliance is the easier thing to hold together and the harder thing to hide. Parties who have declared to each other can plan properly, verify, rehearse, and withdraw without being murdered for it - all the things a conspiracy cannot do. What they cannot do is assemble quietly. Houses that stop feuding are noticed; a grant honoured out of season is noticed; the Earth Vein Tower in particular holds an instrument that makes concealment structurally difficult, and the region contains three institutions whose entire trade is noticing.',
    soTheApexGetsToMobilise:
        'And a warned apex is a different problem from a surprised one. It has courts - the Earth Vein Tower has two, the Myriad Course Hall one - and those courts are institutions at thirty-seven and thirty-eight with their own people, their own ground and every reason to keep the name above them standing. It has a second at thirty-seven to thirty-nine. And it has clients, several of whom are holding sealed ancestors, which the apex may ask them to wake.',
