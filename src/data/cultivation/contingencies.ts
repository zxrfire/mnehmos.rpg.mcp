 ...
 *                   absence to work in.
 *
 *   `sects.ts`      the Frostmirror Sect has a dormant ancestor under its
...
export const CONTINGENCIES: readonly Contingency[] = [
    {
        ...
        theCounterMove:
            'The way to disarm a holder of a sealed ancestor is not to beat them. It is to threaten them hard enough that they burn the instrument defensively - a rival who understood the position could force the Mirror to be spent on survival and walk away having neutralised a two-thousand-year asset without ever facing it, at the cost of a war they intended to lose anyway. Somebody has worked this out: the House of the Narrow Hour holds it as an abstract reading, has published it to nobody, and quietly declined a commission that would have amounted to selling it - one of the two declined commissions in its own records, and the party that commissioned it has no idea how close it came. The Storm Tyrant Sect, which has raided the glacier twice and stopped, would use it the day it heard it and has not heard it.',
        theWakingItself:
