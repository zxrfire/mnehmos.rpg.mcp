...
export const CONTINGENCIES: readonly Contingency[] = [
    {
        ...
        theWatching:
            'Somebody at Frostmirror has been watching for the Survey to move, continuously, for a hundred and ninety years. In practice this is one Core Formation disciple resident at Emerald Water City at any given time, swapped out for the next when they outgrow the post, filing nothing anybody has intercepted - a posting the Court treats as its most important and describes internally as nothing at all. A tour can run decades, because Core Formation is a long realm and there is no hurry about it, so an Emerald Water native could know the same face for most of a mortal life and take it for a fixture. What is unbroken is the watch rather than the watcher. The watching is itself a quiet fact that could be noticed by anybody who wondered why an ice sect keeps a permanent resident in a province where its curriculum cannot be practised.',
        targetAwareness:
        ...
        alliedHolders: [
            ...
            },
            {
                ...
                wouldTheyWakeForThePatron:
                    'They could not if they wanted to. There is no procedure, no seal, no condition and no acknowledgement that there is anything down there - the Sect treats it as a story its ascetics tell each other. It is the only sealed ancestor in the province that might intervene in something by accident and cannot be asked to intervene in anything on purpose.',
                patronBelieves:
                    'Nothing at all. The Survey holds the Sect as its oldest continuous grant and has never heard the story.',
        ...
        theCounterMove:
            'The way to disarm a holder of a sealed ancestor is not to beat them. It is to threaten them hard enough that they burn the instrument defensively - a rival who understood the position could force the Mirror to be spent on survival and walk away having neutralised a two-thousand-year asset without ever facing it, at the cost of a war they intended to lose anyway. Somebody has worked this out: the Flowing Star Tower holds it as an abstract reading, has published it to nobody, and quietly declined a commission that would have amounted to selling it - one of the two declined commissions in its own records, and the party that commissioned it has no idea how close it came. The Storm Tyrant Court, which has raided the glacier twice and stopped, would use it the day it heard it and has not heard it.',
        theWakingItself:
...
export const OTHERS_WHO_NOTICED: readonly {
    ...
    },
    {
        ...
        whatTheyHave:
            'The observation as a joke. Wanderers who have been refused, ruled against or moved on by a Sluice have a standing line about the office that never comes, and it is old enough that nobody remembers who started it.',
        whatTheyLack:
