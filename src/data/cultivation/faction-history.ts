...
export const FACTION_HISTORY: Record<string, FactionHistory> = {
    ...
    'sect-azure-cloud-pavilion': {
        ...
        whyTheGapIs:
            'It is being read backwards by everybody including itself. The Pavilion reliably turns out Core Formation and has not produced above Nascent Soul in three centuries, which looks like an apex living on inheritance and is the opposite: single-use material made above and addressed to a house that still exists is a supply nobody else has at any price, it is arriving now, and the pipeline is climbing on it. What it lost was never the ability to make people. What it never had is time - three hundred and eighty years is not three thousand, and it is trying to grow a gradient the other two apexes did not have to grow.',
    ...
    'sect-hollow-court': {
        ...
        whyTheGapIs:
            'It produces nobody by construction. It takes no disciples on any terms except evidence that you could reach the last realm, so its production figure is zero while four of the highest people alive sit on the ground it holds - the extreme case of the two metrics disagreeing, and the one where reading production as strength goes furthest wrong. Nothing here is falling, because nothing here is doing anything.',
        whatTheUnlitNodesWere:
