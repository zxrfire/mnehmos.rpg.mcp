...
export const LOST_RECORDS = {
    ...
    somebodyElseMayKnow:
        'An outside party with better archives can know more about what is under your floor than you do, and the mechanism is mundane: reading a seal is a scarce skill and the archival powers have it. The Earth Vein Tower holds site records it sealed itself and never published, the Immovable Mountain Temple certifies seals for a fee and keeps copies, and a Dao house will read a hand nobody local can. So a rival with one good document knows the wake condition, the kind and the remaining window, while the holder knows a ceremony - an enormous, entirely quiet advantage that requires no strength at all. Somebody in this world is sitting on one right now, has never mentioned it, and is waiting to see whether it ever becomes useful.',
    categoryCollapse:
...
export const UNOWNED_ANCESTORS: readonly UnownedAncestor[] = [
    ...
    },
    {
        ...
        awareness: 'forgotten',
        whoKnows:
            'Nobody living. The Earth Vein Tower may hold the original site record among the material it sealed itself and never published, which would make it the only party in the world that could say what is down there - and it has never had a reason to look, because a herb valley two provinces from an arterial is not something a Survey register asks questions about.',
        hazard:
