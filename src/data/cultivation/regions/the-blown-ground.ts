...
export const THE_BLOWN_GROUND: UngovernedGround = {
    ...
    register: {
        ...
        smell: 'hot stone and old leather, and no organic smell of any kind, which visitors from the Yellow Plain find harder to sit with than the heat',
    ...
    whatItCostsTheNeighbours: [
        {
            ...
            what: 'Nothing comes out at the Jade Gorge, and that is the cost: the ground sits behind the province rather than beside it, and the province is the centre because of it.',
            cost: 'The Jade Gorge\'s whole position - four roads in one gorge, a toll on traffic it never generated, and every other province resenting it in the same words - rests on the fact that the fifth road crosses ground nobody administers. The centre is being paid for by a vacuum it does not run, does not want run, and has never had to defend in any document. If the Burial Sands were ever held, the Jade Gorge would find out what it is worth without a chokehold, and no institution in the province has ever put that question in writing.'
        },
        {
...
// UNGOVERNED LOOKUPS Lookups only. Nothing here decides anything - who would win a
// fight over a show, what one is worth, how many it takes to hold one - because
// those are questions for the resolvers and a weight function living in a lore file
// is a second combat system.
...
/** The Burial Sands, in the shape the engine can act on. */
