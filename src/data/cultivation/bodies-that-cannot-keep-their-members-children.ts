...
export const NO_PLACE_FOR_THEIR_OWN: readonly NoPlaceForTheirOwn[] = [
    ...
    },
    {
        ...
        reason: 'no intake at all',
        whyItCannotKeepThem:
            'The same absence of a door, moved four provinces and re-signed, and the same reason a favour cannot reach it: a word moves a bar, and there is no bar. Nobody joins the Root Sill either; appointment is by the Long Cut, or by a sect under it or friendly to it, and a child cannot be appointed. What is different is only who signs, and the admission figure on its own row is what a posting requires rather than what an applicant could meet - there has been no applicant in nine hundred years because there is no way to be one.',
        whereTheChildGoes:
            'Home, by the same three routes and into a different kind of house. Sent by the Long Cut, the child goes to the Long Cut - which means to a face rather than to a sect, into an arrangement that ranks people by faces worked and deaths avoided, so what they get is not a rank but a schedule, and a schedule is a thing that can be honoured exactly. Sent by a sect below, the child goes back to that sect. Promoted into the apex, the appointee asks a favour like anybody else. The Root Sill has never been able to explain the first of the three to anybody from the Jade Gorge.',
        whatTheChildKnows:
