...
// A ROUTE
//
// The shape is the catalog's own. The Four Names Terminal in `regions.ts` is
 ...
 * `doesHere` and nothing else, so the Four Names Terminal's far end, its
...
export const SPAN_COUNTER_CATALOG_GAP = {
    ...
    whereItWouldGo: 'RegionBranchSchema in src/data/cultivation/regions.ts, whose Four Names Terminal entry already states a far end, a walked distance and a schedule in prose.',
    whatItWouldTake: 'Optional structured fields on a branch - the far place, the walked days the span replaces, and periodDays/openDays/phaseDay - plus a branch in each major city. The Four Names entry is the worked example and needs no new prose.',
