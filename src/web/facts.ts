...
function timeSkipProse(
    ...
    if (after.satiety <= LOW_SATIETY && after.alive) {
        closing.push(`You are hungry: the belly is down to ${after.satiety} of 100.`);
    }
...
export interface SomebodyInTheSquare {
    ...
    /**
     * What was taken off them that left them short (nothing on, nothing to fight with), and what
     * they came by on the day they did. Facts the narrator plays and the person acts on. See
     * `whatTheCardSaysOfALoss`.
     */
    whatTheyLost?: readonly string[] | null;
    /**
...
function whatTheStretchCostTheBody(after: Cultivator, skip: TimeSkipResult): string[] {
    ...
    if (after.satiety <= LOW_SATIETY && after.alive) {
        said.push(`You are hungry: the belly is down to ${after.satiety} of 100.`);
    }
