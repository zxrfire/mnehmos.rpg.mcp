...
[`docs/world/writing/the-shape-of-play.md`](docs/world/writing/the-shape-of-play.md).
...
**See `src/engine/world/a-house-and-who-is-in-it.md`.** A roster is who a player
could come to know rather than how many the house has; the bands are sampled and
the postings are complete; an office is an elder with a posting. It is a rule
about this world rather than about working in this repo, so it lives beside the
code.
...
Western fantasy. A reader who applies the second set to the first gets the material
wrong in a specific and predictable direction: it treats the genre's ordinary
furniture as atrocity, and softens it.

What is routine here: a tomb is found and everybody runs in. The dead are looted and
their remains are worth something. A body is fought over and claimed. A cultivator is
made to swallow something, made to hand over what they have, made to sit an art they
did not choose, stripped of the core that is the whole of what they are. Sects go to
war over a cave. None of that is a transgression the story pauses on. It is Tuesday,
and the genre's own protagonists do most of it.

This is not an argument for cruelty and it is not a licence to write gratuitously. It
is a statement about what the setting IS, and it has two concrete consequences for
code in this repository.

**A reader may not soften an act into a milder one.** `parseIntent` and the phase-1
classifier route what was said. "I take his purse" is a theft, "I force her to marry
me" is a coercion, and reading either as something politer is not caution - it is the
engine never learning what was tried, and the account that should have opened against
the player never opening. The phase-1 prompt names this as the one failure that cannot
be recovered downstream, and it is the reason. Route it, and let the engine be the
thing that says no.

**The narrator may not editorialise.** Prose that flinches at an ordinary act of the
setting is telling the player the world disapproves, and the world has no opinion.
That is the epigraph at the top of this file, applied to a sentence: 天道无情. A
rescue and a killing are the same kind of event, each has effects, and the effects
follow from what was done rather than from what it deserved.

The people, on the other hand, are not heaven. They beg, resent, are grateful, and
are afraid, and a house that catches you answers - as `what-a-house-does-when-it-
catches-you.ts` already does. Consequence is the whole of the world's response, and
it is not the same thing as judgement.
...
- "I force her to marry me" reached `propose` at every tier. Not because a reader
  ...
  in the closed set. **A vocabulary that can only say the polite version of an act has
  taken a side**, whatever the code around it does.

So when adding a mechanic, ask the question this repo already asks - is it reachable
by a sentence somebody would type - and then ask it again about the version of that
...
```
src/                      # README.md - the index. Every folder below has one too.
├── web/                  # the front door: sentence -> verb -> engine -> prose.
│                         #   The authority rule as type signatures
├── engine/               # what is true and what follows
│   ├── cultivation/      # THE CORE. Realm ladder, roots, breakthrough, the Price of
│   │                     #   Advancement, deviation, injuries, survival, time-skip.
│   │                     #   Pure functions. No DB, no I/O, no MCP
│   ├── world/            # places, capability predicates, opportunity windows,
│   │                     #   history, lineage, possessions, time
│   ├── social/           # relationships, grudges, knowledge and belief, secrets
...
│                         #   constants - never hardcode those elsewhere
├── storage/              # SQLite and migrations
│   └── repos/            # one class per table. Nothing above this writes SQL
├── server/               # the MCP process
│   ├── consolidated/     # the tool surface. One file per tool, action-routed
│   ├── state/            # the two server-lifetime singletons. The WORLD seed lives here
│   └── transport/        # stdio and HTTP
├── agent/                # provider abstraction, the runtime loop, prompt slices
└── math/, utils/, services/   # helpers with no game knowledge in them
```

**Every folder with code in it has a README**, and its file table is generated from each
file's own opening line by `scripts/folder-index.ts`, so the index cannot drift from the
code. `tests/docs/every-link-resolves.test.ts` checks that every relative link in `docs/`
and in a `src/` README still resolves - 1,932 of them.
...
**Code should be self-documenting.** Name things so the code says what it does, and reserve
comments for what the code cannot say: an exception, a measurement, a defect the shape was
chosen against, an ordering that is load-bearing, a thing the next person will otherwise
"tidy" and break.

If a comment restates the code beneath it, delete the comment. If it explains what a good
name would have explained, fix the name instead.

**This repository is currently the counter-example.** Measured across `src/`: **129,488
comment lines against 187,797 code lines - 0.69, or 41% of the file by line.** One file sits
at 7.46 (1,619 comment lines over 217 of code). Agents match the code they read far more
strongly than the rule they are told, so every long header teaches the next author to write
one. Bringing a file down when you are already editing it is real work, not tidying.
