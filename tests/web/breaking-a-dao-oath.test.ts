...
describe('a dao oath', () => {
    ...
    it('is answered by the sky as well', async () => {
        ...
        await game.newRun('Probe');
        await game.act('I swear a dao oath to the Bound Word');
        ...
        expect(
            ledger(db).some(row =>
                row.cause === 'broken_oath' && row.holder_id === 'house-bound-word'),
            'the Bound Word is holding an account about it'
        ).toBe(true);
    });
    ...
    it('can be sworn by somebody who was never introduced to the house', async () => {
        ...
        await game.newRun('Probe');
        const sworn = await game.act('I swear a dao oath to the Bound Word');
