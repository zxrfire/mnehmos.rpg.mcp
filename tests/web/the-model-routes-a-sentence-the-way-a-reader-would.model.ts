 ...
 *
 * AND IT IS NOT IN THE ORDINARY SUITE, which is why this file is named
 * `.model.ts` rather than `.test.ts`. It ran in the default run until it
 * measured a live model on a shared box, where it can move because a sampler
 * landed differently, because the model was swapped, or because the machine was
 * loaded. None of those is a defect in this repo, and a red result nobody can
 * act on teaches everybody to read a red suite as noise. Run it deliberately:
 *
 *     npx vitest run --config vitest.model.config.ts
 *
 * WHEN: whenever the verb surface or the intent prompt changes, because the
 * model's input is `INTENT_SYSTEM_PROMPT` and that prompt is built from the
 * verb surface - including the partition into verbs that cost the asker
 * something and verbs that do not. Read what moves as a signal about a changed
 * input, not as a failure to fix. `vitest.model.config.ts` carries the worked
 * example, from the night the partition last changed.
 */
