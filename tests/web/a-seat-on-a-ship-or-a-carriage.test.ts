 ...
 * boat = genre flying boat"), a seat carries only what a passenger carries on their
 ...
 * down the arms road from Green Water City, whose river mouth is a lane to Sweet
 ...
 * the first day to Cold Peak (found by sweeping sixty seeds, of which four attacked).
 */
...
describe('a seat on a ship or a carriage', () => {
    it('reads the carriage station at the town, with who keeps it', async () => {
        ...
        expect(done.narration).toMatch(/Green Water City by carriage: 2 days \(3 on foot\), \d+ cash a seat/);
        expect(game.state().run.elapsedDays).toBe(0);
    }, 120_000);

    it.each(['I book a carriage to Green Water City', 'I take a carriage to Green Water City'])(
        'rides a paid seat under a roof, fed on board: "%s"', async said => {
            ...
            expect(after.location).toBe('Green Water City');
    ...
    it('sails the lane from the river mouth, and a band that sees the ship withdraws', async () => {
        ...
        await game.newRun('Rider');
        db.prepare("UPDATE cultivators SET location = 'Green Water City', spirit_stones = 500 WHERE id = ?")
    ...
    it('fights a band big enough to take on a hired carriage, three in the foreground and the rest said', async () => {
        ...
        await game.newRun('Rider');
        db.prepare("UPDATE cultivators SET location = 'Green Water City', spirit_stones = 5000, realm_ordinal = 1 WHERE id = ?")
            .run(game.state().cultivator.id);

        const done = await game.act('I hire a carriage to Cold Peak');
    ...
    it('refuses a seat to more than a passenger carries, and a hired carriage takes it', async () => {
        ...
        const seat = await game.act('I book a carriage to Green Water City');
        ...
        await game.act('I hire a carriage to Green Water City');
        expect(game.state().cultivator.location).toBe('Green Water City');
    }, 120_000);
    ...
    it('says when a lane is not worked, and on what day it is', async () => {
        ...
        const done = await game.act('I take the ship to Cold Peak');
    ...
    it('reads the ways a player asks for a ship or a carriage', () => {
        ...
        expect(parseIntent('I buy a ticket to Sweet Spring Island'))
            .toMatchObject({ action: 'passage', intent: 'buy', target: 'Sweet Spring Island' });
        expect(parseIntent('I book a carriage to Green Water City'))
            .toMatchObject({ action: 'passage', intent: 'buy', target: 'Green Water City' });
        expect(parseIntent('I hire a shod carriage to Green Water City'))
            .toMatchObject({ action: 'passage', intent: 'hire', topic: 'shod carriage' });
