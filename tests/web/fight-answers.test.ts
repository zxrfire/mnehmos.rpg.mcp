...
describe('back off to where', () => {
    it('reads the place out of the sentence', () => {
        expect(whereTheyAreHeaded('I back off toward Clear River Ferry')).toBe('Clear River Ferry');
    ...
    it('carries the destination into the answer', () => {
        const read = whatTheySaidInTheFight('I break off and run for Clear River Ferry');
        expect(read).toEqual({ kind: 'break_off', toward: 'Clear River Ferry' });
    });
});
