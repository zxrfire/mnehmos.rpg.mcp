 ...
 *
 * The worked example throughout is the Four Names Terminal, which is in
...
/** The Four Names span: seventeen walked days, four days open in nine. */
const FOUR_GRAVES: SpanRoute = {
    id: 'span-fourhands',
    fromPlace: 'Four Names',
...
describe('the board is the map, and the absence on it is the information', () => {
    const routes: SpanRoute[] = [
        FOUR_GRAVES,
        {
            id: 'span-near', fromPlace: 'Four Names', toPlace: 'the near yard',
    ...
    it('lists only what leaves from where you are standing', () => {
        const board = boardAt('Four Names', routes, RATE, 0);
    ...
    it('reads nearest first, and reads the same way twice', () => {
        ...
        expect(boardAt('Four Names', routes, RATE, 0)).toEqual(board);
    });
    ...
    it('teaches a place, a distance and a price, which is a map', () => {
        ...
        for (const line of boardAt('Four Names', routes, RATE, 0).lines) {
    ...
    it('says what is not on it, and never leaves the gap unexplained', () => {
        expect(boardAt('Four Names', routes, RATE, 0).limits).toContain('survey');
